...
export default function TransactionsPage() {
  ...
  const [selectedTx, setSelectedTx] = useState(null); // transaction being edited, or null
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button size="sm" onClick={() => { setSelectedTx(null); setShowForm(true); }}>
          + Add
        </Button>
      </div>
      ...
      <div className="bg-white rounded-3xl p-4 mt-4 divide-y divide-gray-100">
        {isLoading ? (
 <>
    {[1, 2, 3, 4, 5].map((i) => <TransactionItemSkeleton key={i} />)}
  </>
          ...
          allTransactions.map((tx) => (
            <TransactionItem
              key={tx.id}
              transaction={tx}
              onClick={(tx) => { setSelectedTx(tx); setShowForm(true); }}
            />
          ))
        )}
        ...
        {isFetchingNextPage && <p className="text-center text-gray-400 py-2 text-sm">Loading more...</p>}
      </div>
